...
//gets users, only admins are able to do this
...
// Change a user's role.Admin = 1, user = 2
...
app.post('/api/nodes', authenticateToken, requireAdmin, async (req, res) => {
  ...
  const hostname = req.body.hostname || 'server.local';
  const status = req.body.status || 'Online';
  try {
    const result = await db.query(
      'INSERT INTO nodes (name, ip, hostname, status) VALUES ($1, $2, $3, $4) RETURNING *',
      [name, ip, hostname, status]
    );
    res.json(result.rows[0]);
  } catch (err) {
    console.log('Error adding node:', err);
...
app.delete('/api/nodes/:id', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    console.log('Error removing node:', err);
...
app.post('/api/roles', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ error: 'couldnt create roles' });
  }
});
...
app.post('/api/leaderboard', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await db.query(
      'INSERT INTO leaderboard (user_id, username, score, difficulty) VALUES ($1, $2, $3, $4) RETURNING *',
      [req.user.id, req.user.username, score, difficulty]
    );
    res.json(result.rows[0]);
  } catch (err) {
...
app.get('/api/leaderboard', authenticateToken, async (req, res) => {
  try {
    //gets top 5 scores
    const result = await db.query(
      'SELECT username, score, difficulty FROM leaderboard ORDER BY score DESC LIMIT 5'
    );
    res.json(result.rows);
  } catch (err) {
...
app.post('/api/feedback', authenticateToken, async (req, res) => {
  ...
  try {
    const result = await db.query(
      'INSERT INTO feedback (user_id, username, message, type) VALUES ($1, $2, $3, $4) RETURNING *',
      [req.user.id, req.user.username, message, type]
    );
    res.status(201).json(result.rows[0]);
  } catch (err) {
    console.log('Error saving feedback:', err);
    res.status(500).json({ error: 'Failed to send in feedback.' });
  }
});
