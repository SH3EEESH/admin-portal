...
export default function UserHub() {
  ...
  //Feedback text state for the feedbacck form
  ...
  //get the current leaderboard when loading the page
  useEffect(() => {
...
//feedback
    const submitFeedback = () => {
      ...
      fetch('/api/feedback', { method :'POST', 
        ...
        body: JSON.stringify({ message: feedbackText, type: 'General' })
      })
      .then(res => res.json())
      .then(() => {
        setFeedbackText('');
      ...
      .catch(err => console.log('Error sending feedback:', err));
    };
  ...
  useEffect(() => {
    ...
    const gameOver = () => {
      ...
      fetch('/api/leaderboard', {
        ...
        body: JSON.stringify({ score:scoreRef.current, difficulty: difficultyRef.current})
      })
      .then(res => res.json())
      .then(() =>{

        return fetch('/api/leaderboard', {headers: { 'Authorization' : 'Bearer ' + token}});

      })
      .then(res => res.json())
      .then(data =>setLeaderboard(data))
      .catch(err => console.log('error showing score:', err));
    };
      ...
      <div style={styles.grid}>
        ...
        <div>
          ...
          <div style={styles.roleCard}>
            ...
            <textarea
              ...
              placeholder="Share you feedback or problems that occured"
              style={{ width: '100%', minHeight: '70px', backgroundColor: '#0d1117', color: '#c9d1d9', border: '1px solid #30363d', borderRadius: '6px', padding: '8px', fontFamily: 'monospace', marginBottom: '10px' }}
              />
              <button 
                onClick={submitFeedback}
                style={{backgroundColor: '#3ebd28', color: '#0d1117', border: '1px solid #3ebd28', borderRadius: '6px', fontWeight: 'bold'}} 
                >
        ...
        <div style={styles.gameCard}>
          ...
          <div style={{marginTop: '20px'}}>
            <h3 style={styles.roleCardTitle}>Leaderboard</h3>
            <ul style={styles.permissionList}>
              {leaderboard.length === 0 && (
                <li style={styles.permissionItem}>No Scores Available yet.</li>
              )}
              {leaderboard.map((entry, i) => (
                <li key={i} style={styles.permissionItem}>
                  {i+1}. {entry.username} - {entry.score} ({entry.difficulty})
                </li>
              ))}
            </ul>
          </div>
